...
BulkSendApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BulkSendApplication")
    ...
    .AddAttribute ("Priority", "Priority of the requests",
                   UintegerValue(0),
                   MakeUintegerAccessor (&BulkSendApplication::m_prio),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("IncastServer",
                   "Whether the node is Incast Server",
                   BooleanValue (false),
                   MakeBooleanAccessor (&BulkSendApplication::m_incastserver),
                   MakeBooleanChecker ())
    .AddTraceSource ("Tx", "A new packet is created and is sent",
                     MakeTraceSourceAccessor (&BulkSendApplication::m_txTrace))
    .AddTraceSource ("Start", "The transfer starts",
                     MakeTraceSourceAccessor (&BulkSendApplication::m_sockStart))
    .AddTraceSource ("Stop", "The transfer stops",
                     MakeTraceSourceAccessor (&BulkSendApplication::m_sockStop))
  ;
  return tid;
}
  ...
  if (!m_socket)
    {
      ...
      tcpSock->SetAttribute("IncastServer", BooleanValue(m_incastserver));
