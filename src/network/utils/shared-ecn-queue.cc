...
NS_LOG_COMPONENT_DEFINE ("SharedEcnQueue");

using namespace std;
namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (SharedEcnQueue);

TypeId SharedEcnQueue::GetTypeId (void) 
{
  static TypeId tid = TypeId ("ns3::SharedEcnQueue")
    .SetParent<DropTailQueue> ()
    .AddConstructor<SharedEcnQueue> ()
    .AddAttribute ("Th",
                   "Instantaneous mark length threshold in packets/bytes",
                   UintegerValue (5),
                   MakeUintegerAccessor (&SharedEcnQueue::m_th),
...
bool 
SharedEcnQueue::DoEnqueue (Ptr<Packet> p)
{
...
void
SharedEcnQueue::Mark (Ptr<Packet> p)
{
