...
void SetupServerTraffic (Ptr<Node> Nd, uint16_t appPort, Time startTime, uint16_t nic) 
{
  for (uint16_t i =1; i<=nic;i++)
  {
  	PacketSinkHelper sink ("ns3::TcpSocketFactory", InetSocketAddress (Nd->GetObject<Ipv4>()->GetAddress(i,0).GetLocal(), appPort));
  	ApplicationContainer sinkApps = sink.Install (Nd);
  	sinkApps.Start(startTime);
  }
}

void SetupClientTraffic(Ptr<Node> Nd,Ptr<Node> ServerNode, uint32_t txsize, uint16_t appPort, Time startTime, uint16_t nic_id){

  BulkSendHelper source ("ns3::TcpSocketFactory",InetSocketAddress(ServerNode->GetObject<Ipv4>()->GetAddress(nic_id,0).GetLocal(), appPort));
...
//Moved build inside
  ...
  for (uint32_t i=0;i<NumberofEndPoints;i++){
   bool multi=false;
   for(uint32_t j=0;j<IncastAggregators.size();j++){ 
           if(nEnd[i] == nEnd[IncastAggregators[j]])
			multi = true;
    }
    if(multi){
    devices = p2p.Install (nEnd[i],nLeaf[i/MachinesPerRack]);
 ...
 for(uint32_t i=0; i <IncastAggregators.size();i++){
  ...
  for (uint32_t k=0;k < IncastRequestsPerNode; k++){
		...
		for(uint32_t j=0;j<ClientIdx.size();j++)
		{
			...
			SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start, j%2 +1);
		}
        	NumIncastFlows++;
	}
  }
