...
static void SpinePacketDropped(uint32_t dev, int32_t oldValue, int32_t newValue)
{
     *SpinePstream->GetStream ()<<"Spine\t"<<"SwitchPort:\t"<<dev<<"\tBytesinQueue:\t"<<newValue<<"\n";
}

static void LeafPacketDropped(uint32_t dev, int32_t oldValue, int32_t newValue)
{
//    *LeafPstream->GetStream ()<<"SwitchID:\t"<<i<<"\tPort:\t"<<j<<"\tBytesinQueue:\t"<<dtqueue->m_bytesInQueue<<"\n";
    *LeafPstream->GetStream ()<<"Leaf\t"<<"SwitchPort:\t"<<dev<<"\tBytesinQueue:\t"<<newValue<<"\n";
...
void SetupSpineSwitch(Ptr<Node> rnode, uint32_t nodeID)
{
  bool set_successful = true;
  Ptr<Ipv4L3Protocol> rnodeip = rnode->GetObject<Ipv4L3Protocol>();
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    queue->TraceConnectWithoutContext("PacketDrop", MakeBoundCallback (&SpinePacketDropped, i));
    }
  NS_ASSERT(set_successful);
}
...
void SetupLeafSwitch(Ptr<Node> rnode, uint32_t nodeID)
{
  bool set_successful = true;
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    queue->TraceConnectWithoutContext("PacketDrop", MakeBoundCallback (&LeafPacketDropped, i));
    }
  NS_ASSERT(set_successful);
}
