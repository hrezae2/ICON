...
#include "ns3/multi-priority-queue.h"
...
uint32_t	FlowSize		  = 1024;
bool 		pktspray 		  = 0;
bool		dctcp			  = 1; 
bool		LinkUtilization		  = 0;
string		StatsFileName		  = "Dummy.txt";
double		interval		  = 0.05;
double          testruntime               = 0.00002;

// Derived Parameters
uint32_t        RequestsPerNode;
...
void Configure_Simulator(){
  ...
  RequestsPerNode = int ((1/InterArrival_delay)*testruntime);
  cout<<"RequestsPerNode :"<<RequestsPerNode<<endl;
  cout<<"Short InterArrival Delay :"<<InterArrival_delay<<endl;
...
void Build_Topology()
{
  ...
  e2e.SetQueue  ("ns3::MultiPriorityQueue",
              n1, *v1,
              n2, *v2,
              n3, *v3);
...
void Setup_Workload(){
  ...
  for (uint32_t k=0;k < RequestsPerNode; k++){
  	...
  	for (uint32_t i=0;i<AppIdx.size(); i=i+2){
      		...
      		SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSize,appPort,Curr_Start,2);
		NumberofFlows++;
...
void SetupServer(Ptr<Node> rnode) 
{
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    Ptr<MultiPriorityQueue> dtqueue = queue->GetObject<MultiPriorityQueue>();
    ...
    set_successful &= dtqueue->SetAttributeFailSafe("ServerNode",      BooleanValue(true));
