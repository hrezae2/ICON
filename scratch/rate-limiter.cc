...
void QueuedPackets(int rnode, uint32_t oldValue, uint32_t newValue)
{
  *Queuestream->GetStream () <<"Node:" <<rnode<<":Time:" << Simulator::Now ().GetSeconds ()<<":Bytes:"<<newValue <<"\n";
}
...
void Configure_Simulator(){
  ...
  NumberofEndPointsLong  = 120;//longNodeFactor * NumberofEndPoints; // each rack has one sender and receiver
  NumberofEndPointsShort = 140;//(NumberofEndPoints - NumberofEndPointsLong)*(1.0-incastFactor);
...
void SetDataRate(Ptr<Node> rnode, int DummyID)
{
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    if(!ptpnetdev) {
	  ...
	  dtqueue->TraceConnectWithoutContext("PacketsInQueue", MakeBoundCallback (&QueuedPackets, DummyID));
	  //ptpnetdev->SetInterframeGap(MicroSeconds(40));
    	   ptpnetdev->SetDataRate(DataRate("312Mbps"));
    }
}
...
void Setup_Workload(){
  ...
  for(uint32_t i=0;i<NumberofRacks;i++)
  {
      ...
      for(uint32_t j=0;j<6/*MachinesPerRack*longNodeFactor*/;j++)
      {
          do {
      ...
      for(uint32_t j=0;j<7/*MachinesPerRack*(1.0-longNodeFactor)*(1.0 - incastFactor)*/;j++)
      {
	 do {
		appNode = x->GetInteger ();
	    } while ((find(AppIdxShort.begin(),AppIdxShort.end(),appNode) != AppIdxShort.end()) || (find(AppIdxLong.begin(),AppIdxLong.end(),appNode) != AppIdxLong.end()));
 ...
 for(uint32_t i=0;i<IncastSenders.size();i++){
 
    SetDataRate(nEnd[IncastSenders[i]], i);
...
main (int argc, char *argv[])
{
  ...
  AsciiTraceHelper asciiTraceHelper;
  oss << "Queue." <<StatsFileName ;
  Queuestream = asciiTraceHelper.CreateFileStream ( oss.str ());
