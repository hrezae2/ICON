...
Ptr<OutputStreamWrapper> SpinePstream;
...
static void SpinePacketDropped(int32_t oldValue, int32_t newValue)
{
  for (uint32_t i=0;i<NumberofSpineSwitches;i++)
  {
	 Ptr<Node> rnode  = nSpine[i];
  ...
  for(uint32_t j = 0; j < rnodeip->GetNInterfaces(); j++) {
    ...
    *SpinePstream->GetStream ()<<"SwitchID:\t"<<i<<"\tPort:\t"<<j<<"\tBytesinQueue:\t"<<dtqueue->m_bytesInQueue<<"\n";
    }
...
void SpineQueueLength(uint32_t oldValue,uint32_t newValue)
{
  // *SpinePstream->GetStream ()
    for (uint32_t i=0;i<NumberofSpineSwitches;i++)
  {
         Ptr<Node> rnode  = nSpine[i];
         Ptr<Ipv4L3Protocol> rnodeip = rnode->GetObject<Ipv4L3Protocol>();
    ...
    //std::cout<<"SwitchID:\t"<<GetTypeId<<"\tPort:\t"<<"\tNumberof bytes from:\t"<<oldValue<<"\tto:"<<newValue<<"\n";
    Ptr<Queue> queue = ptpnetdev->GetQueue();
...
void LeafQueueLength(uint32_t oldValue,uint32_t newValue)
{
  // *SpinePstream->GetStream ()
    for (uint32_t i=0;i<NumberofRacks;i++)
  {
    ...
    Ptr<Queue> queue = ptpnetdev->GetQueue();
    Ptr<DropTailQueue> dtqueue = queue->GetObject<DropTailQueue>();
    *LeafPstream->GetStream ()<<"SwitchID:\t"<<i<<"\tPort:\t"<<j<<"\tBytesinQueue:\t"<<dtqueue->m_bytesInQueue<<"\n";
    }
...
static void LeafPacketDropped(int32_t oldValue, int32_t newValue)
{
  for (uint32_t i=0;i<NumberofRacks;i++)
  {
         Ptr<Node> rnode  = nLeaf[i];
...
void Configure_Simulator(){
 ...
 InterArrival_short = (NumberofEndPointsShort*FlowSizeShort*8) / (load * shortLoadFactor * d_LinkSpeed * NumberofSpineSwitches*NumberofRacks *1000000000);
  InterArrival_incast = (NumberofEndPointsIncast*FlowSizeShort*8*incastDegree) / (load * incastLoadFactor * d_LinkSpeed * NumberofRacks * NumberofSpineSwitches *1000000000); // TODO: Include incast degree in the formula 
 ...
 InterArrival_long = 0;
  ...
  else
  {
        ...
        status &= Config::SetDefaultFailSafe ("ns3::TcpNewReno::ReTxThreshold", UintegerValue(0));
  }
...
void Setup_Workload(){
 ...
 for(uint32_t i=0; i <IncastServer.size();i++){
 	uint32_t dummy = IncastServer[i];
	random_shuffle(AppIdxIncast.begin(),AppIdxIncast.end());
	for(uint32_t j=0;j<incastDegree;j++)
	{
		if(dummy == AppIdxIncast[j]){
			swap(AppIdxIncast[j],AppIdxIncast[AppIdxIncast.size()]);
		}
		Map_Client[dummy].push_back(AppIdxIncast[j]);
	}
  ...
  for (uint32_t k=0;k < IncastRequestsPerNode; k++){
        for (uint32_t i=0;i<AppIdxIncast.size(); i=i+1){
                ServerNodeIdx = IncastServer[i];
                do {
                	appPort = UniformlyRandomly->GetInteger(0,65535);
                   } while (find(Map_Port[ServerNodeIdx].begin(),Map_Port[ServerNodeIdx].end(),appPort) != Map_Port[ServerNodeIdx].end());
                Map_Port[ServerNodeIdx].push_back(appPort);
		...
		for(uint32_t j=0;j<ClientIdx.size();j++)
		{
			ClientNodeIdx = ClientIdx[j];
                	//cout<<"Client:"<<j<<" : "<<ClientNodeIdx<<endl;
			SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start);
		}
        	NumIncastFlows++;
	}
...
void SetupSpineSwitch(Ptr<Node> rnode)
{
  bool set_successful = true;
  Ptr<Ipv4L3Protocol> rnodeip = rnode->GetObject<Ipv4L3Protocol>();
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    queue->TraceConnectWithoutContext("PacketDrop", MakeCallback (&SpinePacketDropped));
    }
  NS_ASSERT(set_successful);
}
...
void SetupLengthSpineSwitch(Ptr<Node> rnode)
{
  bool set_successful = true;
  Ptr<Ipv4L3Protocol> rnodeip = rnode->GetObject<Ipv4L3Protocol>();
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    Ptr<Queue> queue = ptpnetdev->GetQueue();
    Ptr<DropTailQueue> dtqueue = queue->GetObject<DropTailQueue>();
    dtqueue->TraceConnectWithoutContext("bytesinqueue", MakeCallback (&SpineQueueLength));
    }
  NS_ASSERT(set_successful);
}


void SetupLengthLeafSwitch(Ptr<Node> rnode)
{
  bool set_successful = true;
  Ptr<Ipv4L3Protocol> rnodeip = rnode->GetObject<Ipv4L3Protocol>();
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    Ptr<Queue> queue = ptpnetdev->GetQueue();
    Ptr<DropTailQueue> dtqueue = queue->GetObject<DropTailQueue>();
    dtqueue->TraceConnectWithoutContext("bytesinqueue", MakeCallback (&LeafQueueLength));
    }
  NS_ASSERT(set_successful);
}
...
void SetupLeafSwitch(Ptr<Node> rnode)
{
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    queue->TraceConnectWithoutContext("PacketDrop", MakeCallback (&LeafPacketDropped));
    }
  NS_ASSERT(set_successful);
}
...
{
  oss << "Spine." <<StatsFileName ;
  AsciiTraceHelper asciiTraceHelper;
  SpinePstream = asciiTraceHelper.CreateFileStream (oss.str ());
  for (uint32_t i=0;i<NumberofSpineSwitches;i++)
  {
    SetupSpineSwitch(nSpine[i]);
  }
std::ostringstream ossl;
...
void Setup_Queuelength_Hooks()
{
  ...
  SpinePstream = asciiTraceHelper.CreateFileStream (oss.str ());
    for (uint32_t i=0;i<NumberofSpineSwitches;i++)
  {
  SetupLengthSpineSwitch(nSpine[i]);
  }
  std::ostringstream ossl;
  ossl << "Leaf." <<StatsFileName ;
  LeafPstream = asciiTraceHelper.CreateFileStream (ossl.str ());
