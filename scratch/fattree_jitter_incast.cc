...
uint32_t        FlowSizeLong              = 2 * 1024 * 1024;
...
void Configure_Simulator(){
 ...
 // calculate the derived knobs
  ...
  if(longLoadFactor > 0)
  {
  	InterArrival_long = (NumberofEndPointsLong*FlowSizeLong*8) / (load * longLoadFactor * d_LinkSpeed * NumberofEndPoints *1000000000);
  }
  else{
	InterArrival_long = 0;
  }
  InterArrival_short = (NumberofEndPointsShort*FlowSizeShort*8) / (load * shortLoadFactor * d_LinkSpeed * NumberofEndPoints *1000000000);
  InterArrival_incast = (NumberofIncastAggregators*FlowSizeShort*8*incastDegree) / (load * incastLoadFactor * d_LinkSpeed * NumberofEndPoints *1000000000); // TODO: Include incast degree in the formula 
