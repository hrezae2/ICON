...
// Fatter Link
  PointToPointHelper p2pF;
  p2pF.SetDeviceAttribute        ("DataRate",  StringValue ("20Gbps"));
  p2pF.SetChannelAttribute       ("Delay",     StringValue (LinkDelay));
  p2pF.SetQueue  (queueType,
              n1, *v1,
              n2, *v2,
              n3, *v3);
  ...
  for (uint32_t i=0;i<NumberofEndPoints;i++){
   ...
   for(uint32_t j=0;j<IncastAggregators.size();j++){ 
    ...
    if(fat){
    devices = p2pF.Install (nEnd[i],nLeaf[i/MachinesPerRack]);
    interfaces = ipv4.Assign(devices);
    ipv4.NewNetwork();
    }
    else{
